...
\xout{	If $S$ is a multiplicatively closed subset of $\bigcup_{g \in G} ( B_g \setminus \{0\})$, then $\HFrac(S^{-1}B) = \HFrac(B)$. }
\end{remark}
...
Now assume $B$ is not trivially graded and let $d = \deg D$. Let $S$ be a multiplicative set generated by a set of homogeneous elements of $A$ whose degrees generate $G(B)$. {\red Simpler: Let $S = \bigcup_{g \in G} ( A_g \setminus \{0\})$.}
Then note that $S^{-1}D : S^{-1}B \to S^{-1}B$ is nonzero and locally nilpotent, {\red and also homogeneous of degree $d$.
Since $-d \in G(B)$ and} $G(A) = G(B)$, there exist
{\red $a,s \in S$ such that $\deg(\frac{a}{s}) = -d$.  Since $\frac{a}{s} \in \ker(S^{-1}D)$,}
it follows that $\frac{a}{s} S^{-1}D$ is a degree 0 homogeneous locally nilpotent derivation of $S^{-1}B$.
Let $B_{(S)}$ denote the degree 0 subring of $S^{-1}B$ {\red and let $D_{(S)} : B_{(S)} \to B_{(S)}$ be the restriction of  $\frac{a}{s} S^{-1}D$.
Then $D_{(S)}$} is a locally nilpotent derivation 
{\red whose kernel is $B_{(S)} \cap \ker\big( \frac{a}{s} S^{-1}D \big) = B_{(S)} \cap S^{-1}A = A_{(S)}$.}
{\cyan You have to check that $D_{(S)} \neq 0$.}
So $\Frac B_{(S)} = \big( \Frac A_{(S)} \big)^{(1)}$ by Preliminary (2). 
{\red Since Lemma \ref{0dfvfb03eFfj} implies that $\HFrac(B) = \Frac B_{(S)}$ and $\HFrac(A) = \Frac A_{(S)}$, we obtain $\HFrac(B) = (\HFrac A)^{(1)}$.}

\xout{We claim that $\HFrac(S^{-1}B) = \Frac(B_{(S)})$. The $(\supset)$ direction is clear.
For $(\subset)$, consider an arbitrary element $\frac{b_1/s_1}{b_2/s_2}$ of $\HFrac(S^{-1}B)$
(where $b_1,b_2$ are homogeneous elements of $B$, $s_1,s_2 \in S$ and $\deg(b_1/s_1) = \deg(b_2/s_2)$).
Again, since $G(A) = G(B)$, there exists $a'/s' \in S^{-1}B$ such that $\deg(a'/s') = -\deg(b_1/s_1) = -\deg(b_2/s_2)$. It follows that $\frac{b_1/s_1}{b_2/s_2} = \frac{b_1 a' / s_1 s'}{b_2 a' / s_2 s'} \in \Frac(B_{(S)})$ since both numerator and denominator are elements of $B_{(S)}$. Moreover, since $S \subset A$, the same argument shows that $\HFrac(S^{-1}A) = \Frac(A_{(S)})$.}

\xout{Finally,  we have that }

$$\xout{\HFrac(B) = \HFrac(S^{-1}B) = \Frac(B_{(S)}) = \Frac(A_{(S)})^{(1)} = \HFrac(S^{-1}A)^{(1)} = \HFrac(A)^{(1)}}$$
\xout{where the first equality follows from Remark \ref{hfracLoc}. This proves the first part. }

...
{\red According to this definition, $A$ is factorially closed in $B$ $\Leftrightarrow$ $A=B$.  Do you see why?}

A derivation $D : B \to B$ is \textit{locally nilpotent} if for all $b \in D$ there exists an $n \in \Nat$ such that $D^n(b) = 0$.  The set of locally nilpotent derivations of $B$ is denoted $\lnd(B)$. \xout{The set of homogeneous locally nilpotent derivations is denoted $\hlnd(B)$.  }
...
\begin{subsection}\  	
	...
	(3) Let $S \subset B$ be a multiplicative subset of $B$ containing $1$. The map $S^{-1}D : S^{-1} B \to S^{-1} B$ defined by $D(\frac{b}{s}) = \frac{sD(b)-bD(s)}{s^2}$ is a derivation satisfying: {\red You have to assume $S \subseteq B \setminus \{0\}$ otherwise (a) is false. Do you see why?}
...
\begin{proof}
Michael, could you please write the proof?
\end{proof}
...
In this subsection, we collect some results about weighted projective surfaces and combine them with Proposition \ref{mainResult} to prove that additional Pham-Brieskorn varieties are rigid. 	
